...
.PHONY: help create_env apt_deps run analysis bclone video traj multtraj compare sandbox clean_gpu clean help
...
apt_deps:
	...
	sudo apt-get update && sudo apt-get install -y cmake libopenmpi-dev python3-dev zlib1g-dev libgl1-mesa-dri mesa-utils
...
# .PHONY: help create_env apt_deps run analysis video traj multtraj compare sandbox clean_gpu clean help
...
# # Install system dependencies: poppler-utils, ffmpeg, libavcodec-extra, graphviz.
# apt_deps:
# 	@echo "Installing system dependencies..."
# 	sudo apt-get update && sudo apt-get install -y cmake libopenmpi-dev python3-dev zlib1g-dev libgl1-mesa-dri mesa-utils
...
# ## Clean GPU memory by terminating running processes.
# clean_gpu:
# 	@echo "Cleaning GPU memory..."
# 	@for pid in $$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits); do \
# 		if [ "$$pid" != "No running processes found" ]; then \
