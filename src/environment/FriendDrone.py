...
from utils import draw_dashed_circle, load_svg_as_surface, pos_to_cell, intercept_direction, load_best_model, generate_sparse_matrix, can_intercept
...
class FriendDrone:
    ...
    def planning_policy(cls, state, friend_activation_threshold_position: float = 0.7, enemy_activation_threshold_position: float = 0.4):
        ...
        def hold_position(pos, friend_intensity, enemy_intensity, enemy_direction, friends_hold=None,
            ...
            if active_friend_count >= MIN_COMMUNICATION_HOLD:
                ...
                if candidate_intercepts:
                    ...
                    if projection_point.distance_to(INTEREST_POINT_CENTER) > cell_center.distance_to(INTEREST_POINT_CENTER):
                        info = ("HOLD", None, None, None)
                        return info, pygame.math.Vector2(0, 0)
                        
                    if pos.distance_to(projection_point) <= EPSILON:
                        info = ("HOLD INTCPT", None, None, None)
                    ...
                    info = ("GO HOLD INTCPT", defensive_point, chosen_cell_center, None)
            ...
            info = ("HOLD - NO ENOUGH COMM", None, None, None)
        ...
        if not enemy_targets:
            ...
            info, direction = hold_position(pos, friend_intensity, enemy_intensity, enemy_direction, friends_hold)
            info = (info[0], info[1], info[2], friends_hold)
        ...
        # Se nenhum alvo foi atribuído a self, mantém a posição.
        friends_hold = [ (cell, candidate_pos) for (cell, candidate_pos) in friend_candidates if cell not in assigned_friend_cells ]
        info, direction = hold_position(pos, friend_intensity, enemy_intensity, enemy_direction, friends_hold)
        info = (info[0], info[1], info[2], friends_hold)
