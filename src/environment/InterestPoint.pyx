...
from settings import INTEREST_POINT_INITIAL_HEALTH, FONT_FAMILY

cdef class CircleInterestPoint:
    """
    ...
    cdef public float internal_radius
    cdef public float external_radius
    cdef public int health
    cdef public tuple base_color

    def __init__(self, center, float internal_radius, float external_radius, color=(0, 255, 0)):
        self.center = center
        self.internal_radius = internal_radius
        self.external_radius = external_radius
        self.health = INTEREST_POINT_INITIAL_HEALTH
        self.base_color = color

    cpdef draw(self, surface):
        cdef float health_ratio
        cdef int r, g, b, diameter
        cdef object alpha_surface, fill_color, current_color, current_color_2, font, health_text, text_rect

        # Calculate health ratio and clamp between 0 and 1
        ...
        # Blit the alpha surface centered
        ...
        pygame.draw.circle(surface, current_color,
                           ...
                           <int>self.internal_radius, 1)

        # Outer outline with lighter color
        current_color_2 = (<int>(r // 3), <int>(g // 3), 0)
        pygame.draw.circle(surface, current_color_2,
                           (<int>self.center.x, <int>self.center.y),
                           self.external_radius, 1)

        # Render and blit health text
